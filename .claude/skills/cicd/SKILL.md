...
name: cicd
description: >
  Katvan's CI/CD lane, layered on `agex pr`. Delegates lint / open /
  read / reply / delta to agex; adds two extensions vendored from
  ...
  creating PRs in katvan, handling review feedback, polling CI status,
  or the user says "create PR", "review comments", "address feedback",
  "resolve threads". Vendored from `agentculture/steward`'s `cicd`
  skill (renamed from `pr-review` in steward 0.7.0; rebased on agex in
  steward 0.12.0).
---

# CI/CD — Katvan (vendored from steward)
...
Default to **FIX** for: portability complaints (always valid across
the AgentCulture mesh — recurring bug class), test or doc requests,
...
the gate. Katvan isn't yet a registered mesh agent, so the post-merge
IRC ping that Culture's `pr-review` includes is still skipped — that
returns when katvan joins the mesh.
