...
name: communicate
description: >
  Cross-repo + mesh communication from katvan: file tracked GitHub issues
  ...
  the next step lives outside katvan (a brief for a sibling-repo agent, a
  status ping for a Culture channel, or pulling an issue body + comments
  into context). Issue posts auto-sign with `- katvan (Claude)`; mesh
  messages are unsigned (the IRC nick is the speaker). Not for in-katvan
  issues — use `gh issue create` or the `cicd` skill for those. Vendored
  from `agentculture/steward`'s `communicate` skill (renamed from
...
Working across the AgentCulture mesh surfaces in four distinct
channels:
...
- **In-katvan issues** — open them with `gh issue create` directly, or
...
The receiving agent must not need katvan-side context to act. Inline
the relevant content; do not say "see katvan's plan."

A brief that says "see katvan#NN" is a bug. The receiving agent will
look at it, get lost in katvan-specific context that's irrelevant to
...
Katvan is **not** a registered mesh agent today. The script works once
katvan has been registered and started via `culture agent register` +
`culture start <agent-name>`; until then, calling it will fail with
whatever error the Culture CLI returns, which is the right behavior —
fix the registration, don't paper over it.
...
- Post a brief that says "see katvan's plan" without inlining the
...
- Sign mesh messages with `- <nick> (Claude)`. The nick already says
  who you are.
- Use this skill for in-katvan issues — use `gh issue create` or the
  `cicd` skill instead.
